...
def get_face_swapper() -> Any:
    ...
    with THREAD_LOCK:
        if FACE_SWAPPER is None:
            ...
            #print(f"🧠 Inicializando FACE_SWAPPER con providers: {roop.globals.execution_providers}")
            FACE_SWAPPER = insightface.model_zoo.get_model(model_path, providers=roop.globals.execution_providers)
            #print("🚀 Face Swapper initialized with providers:", FACE_SWAPPER.session.get_providers())
...
def process_frames(source_path: str, temp_frame_paths: List[str], update: Callable[[], None]) -> None:
    ...
    #print("reference_face:", reference_face)
    if reference_face is None:
        FACES_DETECTED_GLOBALLY=True
    else:
        
        FACES_DETECTED_GLOBALLY=False

        
    for temp_frame_path in temp_frame_paths:
        temp_frame = cv2.imread(temp_frame_path)
        if temp_frame is None:
            print(f"⚠️ Frame inválido (NoneType) en {temp_frame_path}. Saltando este frame.")
            continue  # 🔥 saltar si no se puede leer el frame
        result = process_frame(source_face, reference_face, temp_frame)
        cv2.imwrite(temp_frame_path, result)
        if update:
            update()
    #print("FACES_DETECTED_GLOBALLY:", FACES_DETECTED_GLOBALLY)
...
def process_video(source_path: str, temp_frame_paths: List[str]) -> bool:
    ...
    #print("FACES_DETECTED_GLOBALLY:", FACES_DETECTED_GLOBALLY)
    if FACES_DETECTED_GLOBALLY is False:
